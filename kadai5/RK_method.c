...
double RK_method (double x, double y, double h, int step);
...
double RK_method (double x, double y, double h, int step){
    ...
    for(int i = 0; i < step; i++){
        double x_i1 = x_i + h;
        double k1 = h * diff_equa(x_i, y_i);
        double k2 = h * diff_equa((x_i + h * 0.5), (y_i + k1 * 0.5));
        double k3 = h * diff_equa((x_i + h * 0.5), (y_i + k2 * 0.5));
        double k4 = h * diff_equa((x_i + h), (y_i + k3));

        double y_i1 = y_i + ((k1 + 2 * k2 + 2 * k3 + k4) / 6);
        x_i = x_i1;
        y_i = y_i1;
    }
    calculated_x = x_i;
